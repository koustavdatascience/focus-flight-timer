...
4. In Supabase **Authentication → URL Configuration**, ensure the production application URL and relevant development/preview return URLs are listed as allowed redirect URLs. The current production URL is `https://project-waypoint-app.vercel.app/`; retain `https://focus-flight-timer.vercel.app/` temporarily during the transition.
