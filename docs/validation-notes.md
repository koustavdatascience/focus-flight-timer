...
## 2026-08-15 — Project Waypoint authentication transition and GitHub metadata

The external authentication transition is complete. The Supabase Site URL and redirect allow-list now include `https://project-waypoint-app.vercel.app` while the former production address remains temporarily permitted. A dedicated Project Waypoint Google Cloud project and replacement OAuth web client were configured, and the replacement credentials were saved in the Supabase Google provider. Google sign-in and password recovery were both completed on the new production address; the live application subsequently rendered an authenticated session at that address.

The public GitHub repository About card was also updated from legacy FocusFlight copy. Its current description identifies **Project Waypoint** as a Pomodoro timer with considered virtual flights, and its homepage now links to `https://project-waypoint-app.vercel.app`. The repository API response and a visual check of the public GitHub page confirm the corrected description and destination link.
